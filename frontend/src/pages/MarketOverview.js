...
import WhaleAlertsFeed    from '../components/market-overview/WhaleAlertsFeed';
import StablecoinFlowChart from '../components/market-overview/StablecoinFlowChart';
import TokenUnlocksTable   from '../components/market-overview/TokenUnlocksTable';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      {items.slice(0, 8).map((item, i) => (
        <div key={item.s} style={{
          ...
          onMouseEnter={e => e.currentTarget.style.background = 'rgba(79,124,255,0.07)'}
...
function CryptoTab({ isMobile, rt }) {
  ...
  // 3-col grid template
  const grid3 = isMobile ? '1fr' : '1.35fr 0.85fr 0.75fr';
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      <RunningTicker items={ticker} color={C.crypto} />

      {/* ── Row 1: Liq Heatmap | Derivatives | Whale Alerts ── */}
      <div style={{ display:'grid', gridTemplateColumns:grid3, gap:14, alignItems:'stretch' }}>
              ...
              {d.funding.map(f => (
                <div key={f.s} style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  ...
                  <div style={{ flex:1, height:4, borderRadius:2, background:C.card3, margin:'0 8px', overflow:'hidden' }}>
                    <div style={{ width:`${Math.min(100,Math.abs(f.r)/0.025*100)}%`, height:'100%', background:f.r>=0?C.green:C.red, float:f.r>=0?'left':'right' }} />
                  </div>
                  <span style={{ color:f.r>=0?C.green:C.red, fontSize:11, fontFamily:'monospace', width:54, textAlign:'right' }}>
      ...
      {/* ── Row 2: Crypto Heatmap | Whale Alerts | Stablecoin Flow ── */}
      <div style={{ display:'grid', gridTemplateColumns:grid3, gap:14, alignItems:'stretch' }}>
        ...
        {/* 🐋 Whale Alerts Feed */}
        <SectionBox title="🐋 Whale Alerts" badge="LIVE" extra="On-chain">
          <WhaleAlertsFeed />
        </SectionBox>

        {/* Stablecoin Supply Flow */}
        <SectionBox title="Stablecoin Flow" badge="USDT/USDC" extra="Exchange Netflow">
          <StablecoinFlowChart />
        </SectionBox>
      </div>

      {/* ── Row 3: Token Unlocks Calendar (full width) ── */}
      <SectionBox
        title="Token Unlocks Calendar"
        badge="Next 45 Days"
        extra="Mock · connect API to unlock"
      >
        <TokenUnlocksTable />
      </SectionBox>
        ...
        <SectionBox title="DXY vs Gold Correlation" badge="90D" extra="Inverse Relationship">
          ...
          <div style={{ display:'flex', gap:20, marginTop:8 }}>
            {[
              ...
              { label:'DXY',  value:'100.12',    chg:-0.23, col:'#4F7CFF' },
            ].map(row => (
              <div key={row.label}>
      ...
      <SectionBox title="Performance Matrix" badge="Price Return">
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
            ...
            <tbody>
              {d.perf.map((row, i) => (
                <tr key={row.name} style={{ background:i%2===0?'transparent':'rgba(79,124,255,0.04)' }}>
...
// ── MAIN COMPONENT ────────────────────────────────────────────────────────────
export default function MarketOverview({ t = (k) => k }) {
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding: isMobile ? '14px 14px 0' : '18px 24px 0', flexWrap:'wrap', gap:8 }}>
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <span style={{ background:'#1A2C42', border:`1px solid ${C.border}`, color:C.text3, fontSize:10, padding:'2px 8px', borderRadius:5, fontFamily:'monospace' }}>LIVE</span>
          </div>
