...
const PLACEHOLDER_SLOTS = [
  { ticker: '——', pct: 0, color: COLORS[0] },
  { ticker: '——', pct: 0, color: COLORS[1] },
  { ticker: '——', pct: 0, color: COLORS[2] },
  { ticker: '——', pct: 0, color: COLORS[3] },
  { ticker: '——', pct: 0, color: COLORS[4] },
];
  ...
  return (
    <div style={{ position: 'relative', width: '110px', height: '110px', flexShrink: 0 }}>
      <svg viewBox="-1 -1 2 2" style={{ width: '100%', height: '100%', transform: 'rotate(-90deg)', overflow: 'visible' }}>
        {isEmpty ? (
          /* Empty state — dashed gray ring */
          <circle r="0.825" cx="0" cy="0" fill="none"
            stroke="rgba(79,124,255,0.1)" strokeWidth="0.36"
        ...
        <circle r="0.65" cx="0" cy="0" fill="#111C30" style={{ pointerEvents: 'none' }} />
      </svg>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', textAlign: 'center', gap: '1px' }}>
        {isEmpty ? (
          <span style={{ color: '#1A2C42', fontSize: '18px', lineHeight: 1 }}>+</span>
        ) : displayData && (
          <>
...
export function AssetClassCard({ assets, getLivePrice, grandTotalUSD, kursIdr }) {
  ...
  return (
    <div className="donut-card" style={{ background: '#111C30', border: '1px solid rgba(79,124,255,0.1)', flex: '0.8', minWidth: '160px', flexDirection: 'column', alignItems: 'stretch', gap: 0, padding: '16px 18px' }}>
      <span style={{ color: '#94A3B8', fontSize: '10px', fontWeight: 700, marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block' }}>
        Asset Allocation
      </span>
          ...
          return (
            <div key={i} style={{ opacity: isEmpty ? 0.35 : 1, transition: 'opacity 0.3s' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                ...
                <span style={{ color: isEmpty ? '#4A5568' : '#F8FAFC', fontSize: '11px', fontWeight: 700, fontFamily: 'monospace' }}>
                  {isEmpty ? '—' : `${c.pct.toFixed(1)}%`}
                </span>
              </div>
              <div style={{ width: '100%', height: '3px', backgroundColor: 'rgba(79,124,255,0.06)', borderRadius: '99px', overflow: 'hidden' }}>
                <div style={{
...
export function PositionsCard({ assets, getLivePrice, grandTotalUSD, kursIdr }) {
  ...
  const listItems  = isEmpty ? PLACEHOLDER_SLOTS : pieData;

  return (
    <div className="donut-card" style={{ background: '#111C30', border: '1px solid rgba(79,124,255,0.1)', flex: '0.8', minWidth: '200px', flexDirection: 'column', alignItems: 'stretch', gap: 0, padding: '16px 18px' }}>
      <span style={{ color: '#94A3B8', fontSize: '10px', fontWeight: 700, marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block' }}>
        Positions
      </span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px', maxHeight: '110px', overflowY: 'auto', paddingRight: '2px' }}>
          {listItems.map((d, i) => (
            <div
              key={isEmpty ? i : d.ticker}
              onMouseEnter={() => !isEmpty && setHovered(d.ticker)}
              onMouseLeave={() => !isEmpty && setHovered(null)}
              style={{
                display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                padding: '3px 5px', borderRadius: '4px',
                background: !isEmpty && hovered === d.ticker
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                ...
                <span style={{ color: '#CBD5E1', fontSize: '10px', fontWeight: 700, fontFamily: 'monospace', maxWidth: '44px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {d.ticker}
                </span>
              </div>
              <span style={{ color: '#94A3B8', fontSize: '10px', fontWeight: 600, fontFamily: 'monospace' }}>
                {isEmpty ? '—' : `${d.pct.toFixed(1)}%`}
              </span>
            </div>
          ))}
        </div>
