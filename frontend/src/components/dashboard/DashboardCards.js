...
export function MacroPulseCard({ marketData }) {
  ...
  return (
    <div style={{ background: '#111C30', border: '1px solid rgba(79,124,255,0.12)', borderRadius: '16px', padding: '18px 20px', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
        <div>
          <div style={{ color: '#F8FAFC', fontSize: '14px', fontWeight: 700, letterSpacing: '-0.2px' }}>Macro Pulse</div>
          <div style={{ color: '#4A5568', fontSize: '10px', marginTop: '2px' }}>Updated {now}</div>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0', flex: 1 }}>
        {sectors.map(({ icon, label, s }) => {
          ...
          return (
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 0', borderBottom: '1px solid rgba(79,124,255,0.05)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
                <div style={{ width: '44px', height: '3px', background: 'rgba(79,124,255,0.08)', borderRadius: '99px', overflow: 'hidden' }}>
                ...
                <span style={{ color: '#F8FAFC', fontSize: '13px', fontWeight: 800, fontFamily: 'monospace', minWidth: '20px', textAlign: 'right' }}>
                  {s}<span style={{ color: '#4A5568', fontSize: '9px', fontWeight: 400 }}>/100</span>
                </span>
              </div>
            </div>
          );
...
export function AssetClassPerformanceCard({ marketData }) {
  ...
  return (
    <div style={{ background: '#111C30', border: '1px solid rgba(79,124,255,0.12)', borderRadius: '16px', padding: '16px 20px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
      <div style={{ color: '#94A3B8', fontSize: '10px', fontWeight: 700, letterSpacing: '0.8px', textTransform: 'uppercase', marginBottom: '16px' }}>Asset Class Performance (24H)</div>
        ...
        {classes.map(({ key, icon, accentColor, change }) => {
          ...
          return (
            <div key={key} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div style={{ flex: 1, height: '4px', background: 'rgba(79,124,255,0.06)', borderRadius: '99px', overflow: 'hidden' }}>
...
export function PortfolioMoversCard({ assets, hargaMap, hargaSaham, marketData }) {
  ...
  return (
    <div style={{ background: '#111C30', border: '1px solid rgba(79,124,255,0.12)', borderRadius: '16px', padding: '18px 20px', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
        <div>
          <div style={{ color: '#F8FAFC', fontSize: '14px', fontWeight: 700, letterSpacing: '-0.2px' }}>Portfolio Movers</div>
          <div style={{ color: '#4A5568', fontSize: '10px', marginTop: '2px' }}>Updated {now}</div>
        </div>
        <span style={{ color: '#64748B', fontSize: '9px', fontWeight: 700, background: 'rgba(79,124,255,0.06)', border: '1px solid rgba(79,124,255,0.12)', padding: '2px 7px', borderRadius: '4px', letterSpacing: '0.5px' }}>TODAY</span>
      </div>

      {movers.length === 0 ? (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 6 }}>
          <span style={{ fontSize: '22px' }}>📊</span>
          <span style={{ color: '#4A5568', fontSize: '12px', fontWeight: 500 }}>Add assets to see movers</span>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', flex: 1 }}>
          <div>
            ...
            {gainers.length === 0
              ? <div style={{ color: '#4A5568', fontSize: '11px', padding: '6px 0' }}>—</div>
              : gainers.map(m => (
                <div key={m.ticker} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid rgba(79,124,255,0.05)' }}>
