          ...
          {items.map(({ label, val, sub, fmt }, idx) => (
            <div
              ...
              style={{
                ...
                borderRight: idx % 2 === 0 ? '1px solid rgba(79,124,255,0.06)' : 'none',
                borderBottom: idx < 2 ? '1px solid rgba(79,124,255,0.06)' : 'none',
                display: 'flex', flexDirection: 'column', justifyContent: 'center',
              }}
            >
              <span style={{ color: '#94A3B8', fontSize: '10px', textTransform: 'uppercase', fontWeight: 700, letterSpacing: '0.5px' }}>{label}</span>
              <span style={{ color: val >= 0 ? '#10B981' : '#EF4444', fontSize: '16px', fontWeight: 800, margin: '4px 0 2px' }}>
                {val >= 0 ? '+' : ''}{fmt(val)}
              </span>
              <span style={{ color: '#64748B', fontSize: '11px', fontWeight: 500 }}>{val >= 0 ? '+' : ''}{sub}</span>
            </div>
          ))}
        </div>
      ) : (
